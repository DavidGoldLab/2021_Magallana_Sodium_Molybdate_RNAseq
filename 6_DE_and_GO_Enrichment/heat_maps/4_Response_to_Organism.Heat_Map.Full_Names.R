...
if (file.exists("4_Response_to_Organism.Heat_Map.Full_Names.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("4_Response_to_Organism.Heat_Map.Full_Names.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("4_Gene_Matrix.SM.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="4_Response_to_Organism.Heat_Map.Full_Names.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("4_Response_to_Organism.Heat_Map.Full_Names.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "4_Response_to_Organism.Heat_Map.Full_Names.log2.centered" ) )
dev.off()

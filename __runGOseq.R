...
factor_labeling = read.table("List_SM1-38D_vs_SM3-51D.txt", row.names=1)
...
# get gene lengths
gene_lengths = read.table("goseq.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("All_DE_Genes.txt", header=T, row.names=1)
...
GO_info = read.table("goseq.go_annotations.txt", header=F, row.names=1,stringsAsFactors=F)
...
go_enrich_filename = paste("List_SM1-38D_vs_SM3-51D.txt", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("List_SM1-38D_vs_SM3-51D.txt", '.GOseq.depleted', sep='')
